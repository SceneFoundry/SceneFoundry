...
//          vkDestroyImageView(m_pgpudevice->device(), iv, nullptr);
//       }
//       m_swapChainImageViews.clear();
//
//       // Depth resources
//       for (size_t i = 0; i < m_depthImages.size(); i++) {
//          vkDestroyImageView(m_pgpudevice->device(), m_depthImageViews[i], nullptr);
//          vkDestroyImage(m_pgpudevice->device(), m_depthImages[i], nullptr);
//          vkFreeMemory(m_pgpudevice->device(), m_depthImageMemory[i], nullptr);
//       }
//
//       // Framebuffers
//       for (VkFramebuffer fb : m_swapChainFramebuffers) {
//          vkDestroyFramebuffer(m_pgpudevice->device(), fb, nullptr);
//       }
//       m_swapChainFramebuffers.clear();
//
//       // Render pass
//       vkDestroyRenderPass(m_pgpudevice->device(), m_renderPass, nullptr);
//
//       // Swapchain
//       if (m_swapChain != VK_NULL_HANDLE) {
//          vkDestroySwapchainKHR(m_pgpudevice->device(), m_swapChain, nullptr);
//          m_swapChain = VK_NULL_HANDLE;
//       }
//       // Semaphores and fences
//       for (VkSemaphore sem : m_imageAvailableSemaphores)
//          vkDestroySemaphore(m_pgpudevice->device(), sem, nullptr);
//       for (VkSemaphore sem : m_renderFinishedSemaphores)
//          vkDestroySemaphore(m_pgpudevice->device(), sem, nullptr);
//       for (VkFence fence : m_inFlightFences)
//          vkDestroyFence(m_pgpudevice->device(), fence, nullptr);
//    }
//
//    void sandbox_swap_chain::createSwapChain() {
//       SwapChainSupportDetails swapChainSupport = m_pgpudevice->getSwapChainSupport();
//
...
//       createInfo.surface = m_pgpudevice->surface();
//
...
//
//       QueueFamilyIndices indexes = m_pgpudevice->findPhysicalQueueFamilies();
...
//
//       if (vkCreateSwapchainKHR(m_pgpudevice->device(), &createInfo, nullptr, &m_swapChain) != VK_SUCCESS)
//       {
//          throw std::runtime_error("failed to create swap chain!");
//       }
//       vkGetSwapchainImagesKHR(m_pgpudevice->device(), m_swapChain, &imageCount, nullptr);
//       m_swapChainImages.resize(imageCount);
//       vkGetSwapchainImagesKHR(m_pgpudevice->device(), m_swapChain, &imageCount, m_swapChainImages.data());
//
...
//
//          if (vkCreateImageView(m_pgpudevice->device(), &viewInfo, nullptr, &m_swapChainImageViews[i]) !=
//             VK_SUCCESS)
//          {
...
//
//       if (vkCreateRenderPass(m_pgpudevice->device(), &renderPassInfo, nullptr, &m_renderPass) != VK_SUCCESS)
//       {
...
//          imageInfo.flags = 0;
//
//          m_pgpudevice->createImageWithInfo(
//             imageInfo,
...
//          viewInfo.subresourceRange.layerCount = 1;
//
//          if (vkCreateImageView(m_pgpudevice->device(), &viewInfo, nullptr, &m_depthImageViews[i]) != VK_SUCCESS) {
//             throw std::runtime_error("failed to create texture image view!");
//          }
//       }
//    }
...
//          if (vkCreateSemaphore(m_pgpudevice->device(), &semInfo, nullptr, &m_imageAvailableSemaphores[i]) != VK_SUCCESS ||
//             vkCreateFence(m_pgpudevice->device(), &fenceInfo, nullptr, &m_inFlightFences[i]) != VK_SUCCESS)
//          {
...
//       for (size_t i = 0; i < imageCount; i++) {
//          if (vkCreateSemaphore(m_pgpudevice->device(), &semInfo, nullptr, &m_renderFinishedSemaphores[i]) != VK_SUCCESS) {
...
//    {
//       return m_pgpudevice->findSupportedFormat(
...
//       vkWaitForFences(m_pgpudevice->device(), 1, &m_inFlightFences[m_currentFrame], VK_TRUE, UINT64_MAX);
//
...
//          vkWaitForFences(m_pgpudevice->device(), 1, &m_imagesInFlight[*imageIndex], VK_TRUE, UINT64_MAX);
//       }
//
...
//       vkWaitForFences(m_pgpudevice->device(), 1, &m_inFlightFences[m_currentFrame], VK_TRUE, UINT64_MAX);
//       vkResetFences(m_pgpudevice->device(), 1, &m_inFlightFences[m_currentFrame]);
//
...
//
//       if (vkQueueSubmit(m_pgpudevice->graphicsQueue(), 1, &submitInfo, m_inFlightFences[m_currentFrame]) != VK_SUCCESS) {
...
//
//       VkResult result = vkQueuePresentKHR(m_pgpudevice->presentQueue(), &presentInfo);
//
