...
VkSandboxTexture::VkSandboxTexture(sandbox_device* device) : m_pDevice(device) {}
...
bool VkSandboxTexture::STBLoadFromFile(const ::scoped_string & filename)
{
...
void VkSandboxTexture::KtxLoadCubemapFromFile(const ::scoped_string & name, ::string filename, VkFormat format, sandbox_device* device, VkQueue copyQueue, VkImageUsageFlags imageUsageFlags, VkImageLayout imageLayout)
{
	m_bIsCubemap = true;
...
bool VkSandboxTexture::LoadCubemap(const std::array<::string, 6>& faces) {
	m_bIsCubemap = true;
...
ktxResult VkSandboxTexture::loadKTXFile(::string filename, ktxTexture** target)
{
...
void VkSandboxTexture::fromBuffer(void* buffer, VkDeviceSize bufferSize, VkFormat format, uint32_t texWidth, uint32_t texHeight, sandbox_device* pdevice, VkQueue copyQueue, VkFilter filter, VkImageUsageFlags imageUsageFlags, VkImageLayout imageLayout)
{
	assert(buffer);
