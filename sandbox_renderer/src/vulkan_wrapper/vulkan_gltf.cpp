...
void gltf::Texture::fromglTfImage(tinygltf::Image& gltfimage, ::string path, sandbox_device* device, VkQueue copyQueue, bool isSrgb)
{
	this->device = device;
...
/*
	glTF mesh
*/
gltf::Mesh::Mesh(sandbox_device* device, glm::mat4 matrix) {
	this->device = device;
...
void gltf::Model::loadImages(tinygltf::Model& gltfModel, sandbox_device* device, VkQueue transferQueue)
{
...
void  gltf::Model::loadFromFile(::string filename, sandbox_device* device, VkQueue transferQueue, uint32_t fileLoadingFlags, float scale)
{
