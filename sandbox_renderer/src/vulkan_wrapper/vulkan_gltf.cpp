...
bool loadImageDataFunc(tinygltf::Image* image, const int imageIndex, ::string* error, ::string* warning, int req_width, int req_height, const unsigned char* bytes, int size, void* userData)
{
	...
	if (image->uri.find_last_of(".") != ::string::npos) {
...
void vkglTF::Texture::fromglTfImage(tinygltf::Image& gltfimage, ::string path, sandbox_device* device, VkQueue copyQueue, bool isSrgb)
{
	this->device = device;
...
/*
	glTF mesh
*/
vkglTF::Mesh::Mesh(sandbox_device* device, glm::mat4 matrix) {
	this->device = device;
...
::array_base<VkVertexInputAttributeDescription>   vkglTF::Vertex::vertexInputAttributeDescriptions;
...
::array_base<VkVertexInputAttributeDescription>   vkglTF::Vertex::inputAttributeDescriptions(uint32_t binding, const ::array_base<VertexComponent> components) {
...
VkPipelineVertexInputStateCreateInfo* vkglTF::Vertex::getPipelineVertexInputState(const ::array_base<VertexComponent> components) {
...
void   vkglTF::Model::loadNode( Node* parent, const tinygltf::Node& node, uint32_t nodeIndex, const tinygltf::Model& model, ::array_base<uint32_t>& indexBuffer, ::array_base<Vertex>& vertexBuffer, float globalscale)
{
...
void vkglTF::Model::loadImages(tinygltf::Model& gltfModel, sandbox_device* device, VkQueue transferQueue)
{
...
void vkglTF::Model::loadMaterials(tinygltf::Model& gltfModel)
{
	for (tinygltf::Material& mat : gltfModel.materials) {
		...
		if (itAlpha != mat.additionalValues.end()) {
			const ::scoped_string & mode = itAlpha->second.string_value;
			if (mode == "BLEND") {
...
void  vkglTF::Model::loadFromFile(::string filename, sandbox_device* device, VkQueue transferQueue, uint32_t fileLoadingFlags, float scale)
{
