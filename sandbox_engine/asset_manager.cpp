...
namespace sandbox_engine
{
   ...
   void asset_manager::preloadGlobalAssets() {
      ...
      ::pointer<sandbox_renderer::texture> loadedEnvironmentCubemap = nullptr;
   ...
   void asset_manager::generateIrradianceMap() {
      ...
      VK_CHECK_RESULT(vkCreateImage(m_pgpudevice->device(), &imageCI, nullptr, &irradianceCube->m_image));

      VkMemoryRequirements memReqs;
      vkGetImageMemoryRequirements(m_pgpudevice->device(), irradianceCube->m_image, &memReqs);
      VkMemoryAllocateInfo memAlloc = vkinit::memoryAllocateInfo();
      memAlloc.allocationSize = memReqs.size;
      memAlloc.memoryTypeIndex = m_pgpudevice->getMemoryType(memReqs.memoryTypeBits, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);
      VK_CHECK_RESULT(vkAllocateMemory(m_pgpudevice->device(), &memAlloc, nullptr, &irradianceCube->m_deviceMemory));
      VK_CHECK_RESULT(vkBindImageMemory(m_pgpudevice->device(), irradianceCube->m_image, irradianceCube->m_deviceMemory, 0));
      ...
      VK_CHECK_RESULT(vkCreateImageView(m_pgpudevice->device(), &viewCI, nullptr, &irradianceCube->m_view));
      ...
      VK_CHECK_RESULT(vkCreateSampler(m_pgpudevice->device(), &samplerCI, nullptr, &irradianceCube->m_sampler));
      ...
      VK_CHECK_RESULT(vkCreateRenderPass(m_pgpudevice->device(), &renderPassCI, nullptr, &renderpass));
...

      {
         ...
         VK_CHECK_RESULT(vkCreateImage(m_pgpudevice->device(), &imageCreateInfo, nullptr, &offscreen.image));

         vkGetImageMemoryRequirements(m_pgpudevice->device(), offscreen.image, &memReqs);
         memAlloc.allocationSize = memReqs.size;
         memAlloc.memoryTypeIndex = m_pgpudevice->getMemoryType(memReqs.memoryTypeBits, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);
         VK_CHECK_RESULT(vkAllocateMemory(m_pgpudevice->device(), &memAlloc, nullptr, &offscreen.memory));
         VK_CHECK_RESULT(vkBindImageMemory(m_pgpudevice->device(), offscreen.image, offscreen.memory, 0));
         ...
         VK_CHECK_RESULT(vkCreateImageView(m_pgpudevice->device(), &colorImageView, nullptr, &offscreen.view));
         ...
         VK_CHECK_RESULT(vkCreateFramebuffer(m_pgpudevice->device(), &fbufCreateInfo, nullptr, &offscreen.framebuffer));

         VkCommandBuffer layoutCmd = m_pgpudevice->createCommandBuffer(VK_COMMAND_BUFFER_LEVEL_PRIMARY, true);
         ...
         m_pgpudevice->flushCommandBuffer(layoutCmd, m_transferQueue, true);
      }
      ...
      VK_CHECK_RESULT(vkCreateDescriptorSetLayout(m_pgpudevice->device(), &descriptorsetlayoutCI, nullptr, &descriptorsetlayout));
      ...
      VK_CHECK_RESULT(vkCreateDescriptorPool(m_pgpudevice->device(), &descriptorPoolCI, nullptr, &descriptorpool));

      VkDescriptorSet descriptorset;
      VkDescriptorSetAllocateInfo allocInfo = vkinit::descriptorSetAllocateInfo(descriptorpool, &descriptorsetlayout, 1);
      VK_CHECK_RESULT(vkAllocateDescriptorSets(m_pgpudevice->device(), &allocInfo, &descriptorset));
      ...
      vkUpdateDescriptorSets(m_pgpudevice->device(), 1, &writeDescriptorSet, 0, nullptr);
      ...
      for (uint32_t m = 0; m < numMips; ++m) {
         ...
         for (uint32_t face = 0; face < 6; ++face) {
            ...
            ppipelineIrradiance->set_push_properties(::as_memory_block(pushBlock));
//            vkCmdPushConstants(cmdBuf, irradiancePipeline.getPipelineLayout(), VK_SHADER_STAGE_VERTEX_BIT | VK_SHADER_STAGE_FRAGMENT_BIT, 0, sizeof(PushBlock), &pushBlock);
      ...
      m_pgpudevice->flushCommandBuffer(cmdBuf, m_transferQueue);
      vkQueueWaitIdle(m_transferQueue);

      // cleanup (destroy created renderpass/framebuffer)
      vkDestroyFramebuffer(m_pgpudevice->device(), offscreen.framebuffer, nullptr);
      vkDestroyRenderPass(m_pgpudevice->device(), renderpass, nullptr);
   ...
   void asset_manager::generateBRDFlut() {
      ...
      VK_CHECK_RESULT(vkCreateImage(m_pgpudevice->device(), &imageCI, nullptr, &lutBrdf->m_image));
      VkMemoryAllocateInfo memAlloc = vkinit::memoryAllocateInfo();
      VkMemoryRequirements memReqs;
      vkGetImageMemoryRequirements(m_pgpudevice->device(), lutBrdf->m_image, &memReqs);
      memAlloc.allocationSize = memReqs.size;
      memAlloc.memoryTypeIndex = m_pgpudevice->getMemoryType(memReqs.memoryTypeBits, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);
      VK_CHECK_RESULT(vkAllocateMemory(m_pgpudevice->device(), &memAlloc, nullptr, &lutBrdf->m_deviceMemory));
      VK_CHECK_RESULT(vkBindImageMemory(m_pgpudevice->device(), lutBrdf->m_image, lutBrdf->m_deviceMemory, 0));
      // Image view
      VkImageViewCreateInfo viewCI = vkinit::imageViewCreateInfo();
      ...
      VK_CHECK_RESULT(vkCreateImageView(m_pgpudevice->device(), &viewCI, nullptr, &lutBrdf->m_view));
      // Sampler
      VkSamplerCreateInfo samplerCI = vkinit::samplerCreateInfo();
      samplerCI.magFilter = VK_FILTER_LINEAR;
      samplerCI.minFilter = VK_FILTER_LINEAR;
      ...
      samplerCI.borderColor = VK_BORDER_COLOR_FLOAT_OPAQUE_WHITE;
      VK_CHECK_RESULT(vkCreateSampler(m_pgpudevice->device(), &samplerCI, nullptr, &lutBrdf->m_sampler));
      ...
      VkFramebuffer framebuffer;
      VK_CHECK_RESULT(vkCreateFramebuffer(m_pgpudevice->device(), &framebufferCI, nullptr, &framebuffer));
      ...
      VK_CHECK_RESULT(vkCreatePipelineLayout(m_pgpudevice->device(), &pipelineLayoutCI, nullptr, &pipelinelayout));
      ...
      vkDestroyFramebuffer(m_pgpudevice->device(), framebuffer, nullptr);
      vkDestroyRenderPass(m_pgpudevice->device(), renderpass, nullptr);
