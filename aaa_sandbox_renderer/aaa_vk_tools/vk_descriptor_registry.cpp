...
//#include <map>
//#include <set>
#include "SceneFoundry/graphics3d/include/class_set.h"
...
namespace sandbox_renderer
{
   ...
   void DescriptorRegistry::build(uint32_t maxFrames)
   {
      ...
      // 2) Build our single DescriptorPool via the graphics3d Builder
      sandbox_renderer::sandbox_descriptor_pool::Builder poolBuilder{ m_pgpudevice };
      poolBuilder
      ...
      for (auto& [name, entry] : m_entries) {
         ...
         for (auto& [setIdx, bindings] : bySet) {
            sandbox_descriptor_set_layout::Builder layoutBuilder{ m_pgpudevice };
            for (auto& b : bindings) {
               if (b.count == 1) {
