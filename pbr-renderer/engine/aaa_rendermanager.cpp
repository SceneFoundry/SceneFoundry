...
namespace SceneFoundry_pbr_renderer
{
   ...
   RenderManager::RenderManager(EngineConfig &engineConfig, ::pointer<WindowManager> windowManager,
                                ::pointer<CameraManager> cameraManager) :
       m_pengineconfig(engineConfig), mWindowManager(windowManager), mCameraManager(cameraManager)
   {
   }

   void RenderManager::startup(::pointer<Scene> scene)
   {
      mScene = scene;
      ...
      (void)io;
      io.FontGlobalScale = m_pengineconfig.imguiFontScale;

      ImGui::StyleColorsDark();

      ImGui_ImplGlfw_InitForOpenGL(mWindowManager->getWindow(), true);
      ImGui_ImplOpenGL3_Init(m_pengineconfig.glslVersion);

      // Init any intermediate framebuffers
      mFramebuffer =
         std::make_unique<Framebuffer>(m_pengineconfig.initialViewportWidth, m_pengineconfig.initialViewportHeight);
      mFramebuffer->init();

      mBloomFramebuffers[0] = std::make_unique<BloomFramebuffer>(m_pengineconfig.initialViewportWidth,
                                                                 m_pengineconfig.initialViewportHeight);
      mBloomFramebuffers[0]->init();
      mBloomFramebuffers[1] = std::make_unique<BloomFramebuffer>(m_pengineconfig.initialViewportWidth,
                                                                 m_pengineconfig.initialViewportHeight);
      mBloomFramebuffers[1]->init();

      // Shader
      mPbrShader = std::make_unique<Shader>(m_pengineconfig.engineRootPath + "/src/shaders/pbr.vert",
                                            m_pengineconfig.engineRootPath + "/src/shaders/pbr.frag");
      mBloomShader = std::make_unique<Shader>(m_pengineconfig.engineRootPath + "/src/shaders/bloom.vert",
                                              m_pengineconfig.engineRootPath + "/src/shaders/bloom.frag");
      mPostShader = std::make_unique<Shader>(m_pengineconfig.engineRootPath + "/src/shaders/post.vert",
                                             m_pengineconfig.engineRootPath + "/src/shaders/post.frag");
      mSkyboxShader = std::make_unique<Shader>(m_pengineconfig.engineRootPath + "/src/shaders/skybox.vert",
                                               m_pengineconfig.engineRootPath + "/src/shaders/skybox.frag");

      // Pre-compute IBL stuff
      mIblEquirectangularCubemap =
         std::make_unique<EquirectangularCubemap>(m_pengineconfig.engineRootPath, m_pengineconfig.hdriPath);
      mIblEquirectangularCubemap->compute();

      mIblDiffuseIrradianceMap = std::make_unique<DiffuseIrradianceMap>(m_pengineconfig.engineRootPath,
                                                                        mIblEquirectangularCubemap->getCubemapId());
      mIblDiffuseIrradianceMap->compute();

      mIblSpecularMap =
         std::make_unique<SpecularMap>(m_pengineconfig.engineRootPath, mIblEquirectangularCubemap->getCubemapId());
      mIblSpecularMap->computePrefilteredEnvMap();
      mIblSpecularMap->computeBrdfConvolutionMap();

      glViewport(0, 0, m_pengineconfig.initialViewportWidth,
                 m_pengineconfig.initialViewportHeight); // set initial viewport size
   ...
   void RenderManager::render()
   {
      ...
      ImGui::SameLine();
      ImGui::RadioButton("Both", &mBloomDirection, 0);
      ImGui::SameLine();
      ImGui::RadioButton("Horizontal", &mBloomDirection, 1);
      ImGui::SameLine();
