...
Model::Model(std::string path, ::pointer<Material> material, bool flipTexturesVertically) : mMaterialOverride(material) {
    loadModel(path, flipTexturesVertically);
}
...
Model::loadMaterialTexture(aiMaterial* material, aiTextureType type) {
    ...
    mTexturesLoaded.insert(std::pair<std::string, ::pointer<Texture>>(path.C_Str(), texture));
